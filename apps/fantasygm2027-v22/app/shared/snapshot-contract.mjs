...
export function validateSnapshot(value, now = Date.now()) {
  ...
  return {schema_version:1, league_id:value.league_id, generated_at:value.generated_at,
    ...
    transactions:Array.isArray(value.transactions)?value.transactions:null,
    pool:Array.isArray(value.pool)?value.pool.map(p=>({fantraxId:p.fantraxId,name:p.name,mlbTeam:p.mlbTeam,availability:['DESERT RATS','OTHER TEAM','FREE AGENT','WAIVERS'].includes(p.availability)?p.availability:'UNKNOWN',positions:p.positions,teamId:p.teamId,teamName:p.teamName,sourceStatus:p.sourceStatus})):[],
    ...
    unavailable:['matchup_scores','starts_used','injury_details',...(Array.isArray(value.transactions)?[]:['transactions']),'pitcher_ratings']};
}
